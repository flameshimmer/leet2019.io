...
//
//We then give 1 candy to the first person, 2 candies 
//to the second person, and so on until we give n candies 
//to the last person.
//
//Then, we go back to the start of the row, giving n + 1 
//candies to the first person, n + 2 candies to the second
//person, and so on until we give 2 * n candies to the last person.
//
//This process repeats (with us giving one more candy each 
//time, and moving to the start of the row after we reach 
//the end) until we run out of candies.  The last person 
//will receive all of our remaining candies (not necessarily 
...
namespace Solution2019
{
	namespace DistributeCandiestoPeople
	{
		vector<int> distributeCandies(int candies, int num_people) {
			...
			int index = 0;
			while (candies) {
				result[index % num_people] += min(candies, cur);
				candies -= min(candies, cur);
				index++;
				cur++;
			}
			return result;
