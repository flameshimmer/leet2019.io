...
//A new event can be added if adding the event will not 
...
//the event can be added to the calendar successfully without
//causing a triple booking. Otherwise, return false and do not
...
//Explanation: 
//The first two events can be booked.  The third event 
//can be double booked.
//The fourth event (5, 15) can't be booked, because it 
//would result in a triple booking.
//The fifth event (5, 10) can be booked, as it does not 
//use time 10 which is already double booked.
//The sixth event (25, 55) can be booked, as the time in 
//[25, 40) will be double booked with the third event;
//the time [40, 50) will be single booked, and the time 
//[50, 55) will be double booked with the second event.
// 
//Note:
//The number of calls to MyCalendar.book per test case
//will be at most 1000.
//In calls to MyCalendar.book(start, end), start and end 
...
namespace Solution2019
{
	...
	{
		namespace RememberOverlapSection {
			class MyCalendarTwo {
			public:
