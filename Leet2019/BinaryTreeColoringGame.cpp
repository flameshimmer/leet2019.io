...
namespace Solution2019
{
	...
	{
		int count(TreeNode* node, int& leftCount, int& rightCount, int val) {
			if (!node) { return 0; }
			int l = count(node->left, leftCount, rightCount, val);
			int r = count(node->right, leftCount, rightCount, val);
			if (node->val == val) { leftCount = l; rightCount = r; }
			return l + r + 1;
		}
		...
		bool btreeGameWinningMove(TreeNode* root, int n, int x) {
			int left = 0;
			int right = 0;
			int total = count(root, left, right, x);
			return max(max(left, right), total - left - right - 1) > total / 2;
		}
